...
class RTXDebug extends MusicBeatState
{
	...
	override function create()
	{
		...
		MusicBeatState.windowNameSuffix = " RTX Menu";
		
		#if DISCORD_ALLOWED
		// Updating Discord Rich Presence
		DiscordClient.changePresence("In the RTX Menu", null);
		#end
		...
		charDropDown = new FlxScrollableDropDownMenu(10, 500, FlxUIDropDownMenu.makeStrIdLabelArray(chars, true), function(character:String)
		{
			remove(char);
			char.kill();
			char.destroy();

			daAnim = chars[Std.parseInt(character)];
		...
		stageGroup = new StageGroup(stage_Name);
		insert(0, stageGroup);

		stageDropDown = new FlxScrollableDropDownMenu(charDropDown.x + charDropDown.width + 1, charDropDown.y, FlxUIDropDownMenu.makeStrIdLabelArray(curStageList, true), function(stage:String)
		{
			remove(stageGroup);
			stageGroup.kill();
			stageGroup.destroy();
		...
		stageDropDown.selectedLabel = stage_Name;
		...
		{
			var rtxData = stageGroup.stageData.rtxData;
	...
	override function update(elapsed:Float)
	{
		...
		{	
			...
			stageGroup.stageData.rtxData = rtxData; 

			var data:String = Json.stringify(stageGroup.stageData, null, "\t");
