...
void main() {
    ...
    gl_FragColor = flixel_texture2D(bitmap, openfl_TextureCoordv) + (vec4(clamp(1.5 * pow(clamp(pow(fbm(uv), 1. + 4. * clamp(uv.y * uv.y, 0., 1.)) * 1.5, 0., 1.) * c.xxx, vec3(1, 3, 6)), 0., 1.), 1.) * strength);
}
