...
function createPost()
    ...
    for i = 0,noteCount-1 do 
		...
		if nt == 'VC Bullet VIP' and not getUnspawnedNoteSustainNote(i) then 
...
function playerOneSing(data, time, noteType) --the	
	if noteType == 'VC Bullet VIP' then
...
function makeTracer()
	...
	makeSprite(sprName, '', getActorX('dad')+400, getActorY('dad')+500)
	...
	setActorAngle(0 + (math.random()*2), sprName)
