...
function update(elapsed)
    ...
    if #punches > 0 then 
        ...
        if punches[1][1]-beatTiming < getSongPosition() then
            --show punch
            ...
            actorScreenCenter('punchesLeft')
            setActorY(300, 'punchesLeft')
            if punchTime-punchEarlyHitTiming < getSongPosition() and getSongPosition() < punchTime+punchLateHitTiming then
                --can be hit during this time
                canBeDodged = true
                if not punches[1][4] and punchTime <= getSongPosition() then 
                    punches[1][4] = true
                    playActorAnimation('punch'..punches[1][2], 'punch'..punches[1][2],true)

                end
                if punchTime <= getSongPosition() then 
                    if bot or opponentPlay then
                        tryDodge(true)
                    end
                end
            end
            if getSongPosition() > punchTime+punchLateHitTiming then --miss
...
function tryDodge(botplay)
    ...
    if canBeDodged then 
        ...
        dodging = false
        dodgeCooldown = 0
        local ms = -math.floor(punchTime-getSongPosition())
        local text = ms.."ms"
        if botplay then 
            text = text.." (BOT)"
        end
