...
local yPositions = {320, 320}  
local xPositions = {3, 18} --1,9,18
...
function createPost()
    ...
    setShaderProperty('bars', 'effect', 0.2)
...
local drunkSpeed = 8
function stepHit()
	if curStep == 10 or curStep == 1664 then 
		for i = 0, (keyCount + playerKeyCount) - 1 do 
			local randomX = ((math.random() * 2) - 1) * 200
			local randomY = (math.random() * -300) - 200
	
			-- Establecer la velocidad del actor
			setActorVelocityX(randomX, i)
			setActorVelocityY(randomY, i)
	
			-- Aplicar aceleración para que caigan con gravedad
			setActorAccelerationY(800, i)
	
			-- Ajustar posiciones iniciales (esto es opcional si ya las modifica la velocidad)
			targetnoteXPos[i + 1] = _G['defaultStrum'..i..'X'] + randomX
			targetnoteYPos[i + 1] = _G['defaultStrum'..i..'Y'] + randomY
		end
	end
...
local flipShit = 1
function doModchartShit(state, modifyY)
    flipShit = flipShit * -1
    if state == 0 then 
        ...
        for i = 0, 7 do
            ...
            if modifyY then
                noteYPos[i + 1] = noteYPos[i + 1] + (20 * factor * direction) * flipShit
            else
                noteZPos[i + 1] = noteZPos[i + 1] - (45 * factor * direction) * flipShit
            end
        end
