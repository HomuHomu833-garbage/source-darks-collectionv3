...
function update(elapsed)
    ...
    a = a + (elapsed * 1.5)
    setActorPos(180 + ((math.cos(a) * 2) * window), 450 + (math.sin(2 * a) * window), "dadCharacter0")
	setActorPos(200 + ((math.cos(a + 1) * 2) * window), 450 + (math.sin(2 * (a + 1)) * window), "dadCharacter1")
    setActorPos(2000 + ((math.cos(a) * 2) * window), 450 + (math.sin(-2 * a) * window), "bfCharacter0")
	setActorPos(2100 + ((math.cos(a + 1) * 2) * window), 450 + (math.sin(-2 * (a + 1)) * window), "bfCharacter1")
    for i=start_Rock_Num, end_Rock_Num do
        local actor_Name = "undefinedSprite" .. tostring(i)
